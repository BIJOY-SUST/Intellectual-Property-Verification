...
async function main(email, rapcKey) {
    try {

        // Create a new file system based wallet for managing identities.
        const walletPath = path.join(process.cwd(), 'wallet');
        const wallet = new FileSystemWallet(walletPath);
        console.log(`Wallet path(Reduce Post Count): ${walletPath}`);
        ...
        await contract.submitTransaction('reducePost',rapcKey);
        // await contract.submitTransaction('createCar', 'CAR15', 'Honda', 'Accord', 'Black', 'BIJOY');

        console.log('Transaction(Reduce Post Count) has been submitted');
