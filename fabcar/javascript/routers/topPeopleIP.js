...
async function main(email) {
    try {
        ...
        console.log(`Wallet path(topPeopleIP): ${walletPath}`);
        console.log('email sent to topIP:' +email)
        ...
        const result = await contract.evaluateTransaction('topPeopleIP',email);
        //console.log(`[top ip] Transaction has been evaluated, result is: ${result.toString()}`);
        return result.toString();


    } catch (error) {
        console.error(`[top ip]Failed to evaluate transaction: ${error}`);
        process.exit(1);
        return error;
    }
}
